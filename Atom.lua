...
local copyLogPath = '' --path that will be used to store the log data
local copyLogFile = 'clipboard.log' --name+extension of the file that will be used to store the log data


local osd_messages = false --true is for displaying osd messages when actions occur, Change to false will disable all osd messages generated from this script
...
function paste()
  ...
  elseif (filePath == nil) and (starts_protocol(protocols, videoFile)) or (filePath ~= nil) and (logVideoTime == nil) then
    if (osd_messages == true) then
      mp.osd_message('Pasted:\n'..videoFile)
    end
    ...
    if (mp.get_property_number('playlist-count')) == 0 then
      ...
      mp.commandv('loadfile', videoFile, 'append')
    end
...
function welcome_screen()
  ...
  msg.info("")
  msg.info("                                   Play/Pause: Space")
  msg.info("                               Volume Up/Down: Up/Down Arrow")
  msg.info("                            Seek Forward/Back: Right/Left Arrow")
